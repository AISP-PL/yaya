...
import numpy as np
from PyQt5.QtGui import QBrush, QPolygon, QColor, QPen, QTransform
from PyQt5.QtCore import Qt
from random import randint
from PyQt5.Qt import QPoint, QFontMetrics
...
def CvImage2QtImage(cvImg):
    ...
    return QPixmap(QImage(np.require(cvImg, np.uint8, 'C'),
                          cvImg.shape[1],
                          cvImg.shape[0],
                          cvImg.shape[1]*3,
...
def QDrawTriangle(painter, point, size=10, angle=0, pen=Qt.black, brushColor=Qt.white, brushStyle=Qt.SolidPattern, brushOpacity=1):
...
def QDrawRectangleWH(painter, point, width=100, height=20, angle=0,
                     pen=Qt.black, brushColor=Qt.white, brushStyle=Qt.SolidPattern, brushOpacity=1):
    ''' Helper function for polygon drawing.'''
...
def QDrawRectangle(painter, points,
                   pen=Qt.black, penThickness=1, penStyle=Qt.SolidLine,
                   brushColor=Qt.white, brushStyle=Qt.SolidPattern, brushOpacity=1):
    ''' Helper function for polygon drawing.'''
    p1 = points[0]
    p2 = points[1]
    polygon = QPolygon()
...
def QDrawText(painter, point, text, fontSize=10, fontBold=False, pen=Qt.white,
              shadow=None, shadowMargin=2, bgColor=Qt.black, bgStyle=Qt.SolidPattern,
              bgOpacity=1, bgMargin=4, textAlign='center'):
    ''' Helper function for polygon drawing.'''
    # Create Font
    font = painter.font()
    ...
    bgrect = CreateRectangle(point.x(), point.y(),
    ...
    p1, p2, p3, p4 = bgrect.point(0), bgrect.point(
        1), bgrect.point(2), bgrect.point(3)
    textPoint = QPoint(min(p1.x(), p2.x()) + bgMargin,
                       max(p1.y(), p3.y()) - bgMargin)

    # Draw shadow
    if (shadow is not None):
        painter.setPen(shadow)
        painter.drawText(textPoint.x()+shadowMargin,
                         textPoint.y()+shadowMargin,
                         text)
...
def QDrawCrosshair(painter, x, y, width, height, penColor=Qt.black, penColorAlpha=1.0, penThickness=1, penStyle=Qt.SolidLine):
...
def QDrawPolyline(painter, pointsList, penColor=Qt.black, penColorAlpha=1.0, penThickness=1, penStyle=Qt.SolidLine, isClosed=False):
    ''' Helper function for polyline drawing.'''
    polygon = QPolygon()
    for point in pointsList:
        polygon.append(point)
...
def QDrawArrow(painter, pointsList, penColor=Qt.black, penColorAlpha=1.0, penThickness=1, penStyle=Qt.SolidLine):
    ''' Helper function for polyline drawing.'''
    # Set pen
    color = QColor(penColor)
    color.setAlphaF(penColorAlpha)
...
def QDrawDistances(painter, distance, penColor=Qt.black, penColorAlpha=1.0, penThickness=1, penStyle=Qt.SolidLine):
    ''' Helper function for polyline drawing.'''
    pointsList = distance['QViewTrajectory']
    # Draw joints
    QDrawJoints(painter, pointsList, 5, Qt.black,
                penColor, Qt.SolidPattern, 0.5)
    # Draw line 1
    QDrawPolyline(painter, pointsList[0:2], penColor,
                  penColorAlpha, penThickness, penStyle)
    # Draw line 2
    QDrawPolyline(painter, pointsList[2:4], penColor,
                  penColorAlpha, penThickness, penStyle)
    # Calculate center line
    tupleList = QPointListToTupleList(pointsList)
    pc0 = algebra.GetMiddlePoint(tupleList[0], tupleList[1])
    pc1 = algebra.GetMiddlePoint(tupleList[2], tupleList[3])
    QDrawPolyline(painter, [QPoint(pc0[0], pc0[1]), QPoint(
        pc1[0], pc1[1])],  penColor, penColorAlpha, penThickness, penStyle)
    # Draw letter at the begining
    QDrawText(painter, QPoint(pc0[0], pc0[1]),
