...
def Rain(image):
    ...
    for i in range(drops):
        ...
        ksize = (size - 1, size - 1)
        # Using cv2.blur() method
        image[y - size : y + size, x - size : x + size] = cv2.blur(
            image[y - size : y + size, x - size : x + size], ksize
        )
...
def Affine(image, factor=0.1):
    ...
    pts2 = np.float32(
        [
            [w * factor, h * factor],
            [
                w * (1 - factor) + w * uniform(0, factor),
                h * factor + h * uniform(0, factor),
            ],
            [
                w * factor + w * uniform(0, factor),
                h * (1 - factor) + h * uniform(0, factor),
            ],
        ]
    )
...
def RandomShapeTransform(image):
    ...
    if method == 0:
        ...
        image = Translate(
            Rotate(image, angle=randint(5, 45)), x=randint(5, 100), y=randint(5, 100)
        )
    elif method == 5:
...
def RandomColorTransform(image):
    ...
    if method == 0:
        ...
        image = AddPattern(
            image,
            alpha=uniform(0.5, 0.9),
            columns=randint(8, 16),
            dotwidth=randint(8, 40),
        )
    elif method == 7:
        image = Rain(image)
    elif method == 8:
